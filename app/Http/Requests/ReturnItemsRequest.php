        ...
        $validator->after(function ($validator) {
            ...
            foreach ($itemIds as $index => $itemId) {
                ...
                if ($qty > $remainingQuantity) {
                    $validator->errors()->add("qty.$index", 'Returned quantity exceeds the remaining returnable quantity.');
                }
        ...
        foreach ($requestedByProduct as $productId => $qty) {
            ...
            if ($qty > $availableQuantity) {
                foreach ($productIndexes[$productId] ?? [] as $index) {
                    $validator->errors()->add("qty.$index", 'Returned quantity exceeds available stock for this product.');
                }
            }
        }
    }
